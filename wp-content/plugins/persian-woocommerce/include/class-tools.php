...
if ( ! class_exists( 'Persian_Wooommerce_Tools' ) ) :

class Persian_Wooommerce_Tools extends Persian_Wooommerce_Plugin {
	...
	public function admin_tabs( $current = 'general', $current_section = "" ) {
		...
		foreach( $tabs as $tab => $name ){
			if($tab == $current){
				...
				$class = "";
			echo "<a class='nav-tab$class' href='?page=persian-wc-tools&tab=$tab'>$name</a>";
			...
			if($tab == $current && isset($sections[$tab])){
				foreach( $sections[$tab] as $section => $name ){
					if($section == $current_section || !count($html_sections)){
						...
						$class = "";
					$html_sections[] = "<li><a href='?page=persian-wc-tools&tab=$tab&section=$section' class='$class'>$name</a></li>";
				}
			}
		}
		echo '</h2>';
...
$PW->tools = new Persian_Wooommerce_Tools();
$PW->tools->load_tools();
